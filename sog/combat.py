...
class Combat:
    ...
    def attackHit(self, attackerObj, defenderObj, attackCmd="attack"):
        ...
        dLog(
            logPrefix
            + "Calculating attackHit for "
            + attackerObj.describe()
            + " attacking "
            + defenderObj.describe()
            + " with cmd "
            + attackCmd
            + "...",
            self._instanceDebug,
        ...
        if random.randint(1, 4) == 1:
            dLog(logPrefix + "  25% hit chance triggered", self._instanceDebug)
            return True
        ...
        if attackCmd in SpellList:
            ...
            dLog(
                logPrefix + "  " + str(attackCmdAdj) + "% magic skill adj",
                self._instanceDebug,
            )
        else:
            ...
            hitPercentage += offenseAdj
            dLog(
                logPrefix + "  " + str(attackCmdAdj) + "% weapon/skill adj",
                self._instanceDebug,
            )
        ...
        dLog(
            ...
            + str(hitPercentage)
            + "% total hit percent "
            + "needs to be less than random roll "
            + str(hitRoll),
            self._instanceDebug,
        )
    ...
    def calcDmgPct(self, attackerObj, opponentObj, attackCmd="attack"):
        ...
        dLog(
            logPrefix
            + "Calculating DmgPct for "
            + attackerObj.describe()
            + " attacking "
            + opponentObj.describe()
            + " with cmd "
            + attackCmd
            + "...",
            self._instanceDebug,
        ...
        dLog(
            logPrefix + "  " + str(damagePercent) + "% starting point",
            self._instanceDebug,
        )
        ...
        if isinstance(attackerObj, Character):
            ...
            if (
                attackerObj.getAlignment() == "chaotic"
                and opponentObj.getAlignment() == "good"
            ):
                damagePercent += 10
                dLog(logPrefix + "  10% alignment bonus", self._instanceDebug)
            elif (
                attackerObj.getAlignment() == "lawful"
                and opponentObj.getAlignment() == "evil"
            ):
                damagePercent += 10
                dLog(logPrefix + "  10% alignment bonus", self._instanceDebug)
            ...
            dLog(
                logPrefix + "  " + str(skillPercent) + "% skill bonus",
                self._instanceDebug,
            )
            ...
            dLog(
                logPrefix + "  " + str(strengthPercent) + "% strength adj",
                self._instanceDebug,
            )
        ...
        dLog(
            logPrefix + "  " + str(cmdPercent) + "% attack cmd bonus",
            self._instanceDebug,
        )

        dLog(
            logPrefix + str(damagePercent) + "% total damage percent",
            self._instanceDebug,
        )
    ...
    def attackDamage(self, attackerObj, opponentObj, attackCmd="attack"):
        ...
        damagePercent = int(
            self.calcDmgPct(attackerObj, opponentObj, attackCmd=attackCmd) / 100
        )
    ...
    def whiskAwayInsteadOfDeath(self, charObj, roomNum=184):
        ...
        * Future??? monster will placed in the room connected to
        ...
        if roomNum == self._kidnapRoomNum:
            msg = (
                "Everything goes black...  As you start to come around, "
    ...
    def attackCreature(self, charObj, target, attackCmd="attack", spellObj=None):
        ...
        if attackCmd != "slay" and self.misses(charObj, target, attackCmd):
            ...
            self.applyDamage(charObj, charObj, target, damage, attackCmd=attackCmd)
        # end attackCreature
            ...
            if creatureObj.isAttacking():
                ...
                for charObj in characterList:
                    if creatureObj.initiateAttack(charObj):
                        self.charMsg(
                            charObj, creatureObj.describe() + " attacks you!\n"
                        )
    ...
    def creatureAttacksPlayer(self, creatureObj, charObj=None):
        ...
        dLog(
            logPrefix + creatureObj.describe() + " is attacking " + charObj.getName(),
            self._instanceDebug,
        )
        ...
        self.applyDamage(charObj, creatureObj, charObj, damage, attackCmd="attack")
        return None
    ...
    def applyDamage(self, charObj, attacker, target, damage, attackCmd):
        ...
        if damage:
            ...
            atkDict = self.getattackMsgDict(charObj, attacker, target, attackCmd)
        ...
        if target.damageIsLethal(damage):
            debugMsg = (
                atkDict["attackerName"]
                + " does lethal damage to "
    ...
    def getHitMsg(self, atkDict, damage):
        ...
        msg = (
            atkDict["msgPrefix"]
            + atkDict["attackerSubject"]
            + " "
            + atkDict["attackerVerb"]
            + " "
            + atkDict["targetSubject"]
            + " for "
            + str(damage)
    ...
    def applyCreatureDamage(self, charObj, target, damage, attackCmd="attack"):
        ...
        if target.damageIsLethal(damage):
            self.charMsg(charObj, "You killed " + target.describe(article="the") + "\n")
            self.othersInRoomMsg(
                charObj,
                roomObj,
                charObj.getName() + " kills " + target.describe() + "\n",
            )
    ...
    def applyPlayerDamage(self, charObj, attacker, damage):
        ...
        if charObj.damageIsLethal(damage):
            if attacker.kidnaps():
                ...
                self.whiskAwayInsteadOfDeath(charObj, roomNum=self._kidnapRoomNum)
            else:
                if not charObj.isDm():
    ...
    def engageTarget(self, charObj, target):
        ...
        if charObj.getCurrentlyAttacking() != target:
            ...
            dLog(
                logPrefix + charObj.describe() + " engages " + target.describe(),
                self._instanceDebug,
            )
            self.othersInRoomMsg(
                charObj,
    ...
    def run(self, charObj):
        ...
        if not charObj.isAttackingWithFist():
            ...
            oMsg = charObj.getName() + " drops " + possessive + " weapon and runs away!"
        else:
            cMsg = "You run away!"
