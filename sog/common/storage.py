...
class Storage:
    ...
    def setDataFilename(self, dfStr=""):
        ...
        if dfStr == "":
            ...
            if not id:
                logger.error(
                    logPrefix + "Could not retrieve Id to " + "generate filename"
                )
                return False

            if id == "":
                logger.error(
                    logPrefix + "ID is empty while generating " + "datafile name"
                )
                return False
            ...
            self._datafile = os.path.abspath(
                DATADIR + "/" + self.__class__.__name__ + "/" + str(id) + extension
            )
        else:
    ...
    def load(self, requestedAttNames=[], logStr="", fileType="pickle"):
        ...
        if filename == "":
            logger.error(
                logPrefix + " Could not determine " + "filename for loading " + logStr
            )
            return False
        ...
        if self.dataFileExists():
            ...
            if self.isValid():
                ...
                logger.error(logPrefix + logStr + str(self.getId()) + " is not valid")
        else:
            logger.warning(
                logPrefix + " " + logStr + "datafile doesn't exist at " + self._datafile
            )
        return False
    ...
    def writeJSonFile(self, filename):
        jsonpickle.set_encoder_options(
            "json", sort_keys=True, indent=4, ensure_ascii=False
        )
    ...
    def delete(self, logStr=""):
        ...
        if not os.path.isfile(filename):
            ...
            return False
        if self.dataFileExists():
            logger.info(logPrefix + " Preparing to delete " + logStr + " " + filename)
            try:
                os.remove(filename)
            except OSError as e:
            ...
            if os.path.isfile(filename):
                logger.error(logPrefix + " " + filename + " could not " + "be deleted")
                return False
            else:
