...
SpellDict = {
    ...
    "lightning": {
        ...
        "intRequired": 13,
        "formula": (
            "min(20 + (2 * (self.charObj.getLevel() + 1)), "
            + "self.charObj.getIntelligence() * 2)"
        ),
        "formulaResultAtt": "damage",
        "levelRequired": {
...
SpellsThatTargetSelfList = [
    spellName for spellName in SpellList if (SpellDict[spellName]["targetSelf"])
]
...
class Spell:
    ...
    def setSpellAttributes(self):
        ...
        for att in SpellDict[self.spellName].keys():
            if att == "levelRequired":
                ...
                dLog(
                    "magic: level="
                    + str(self.charObj.getLevel())
                    + " - Int="
                    + str(self.charObj.getIntelligence()),
                    self._instanceDebug,
                )
    ...
    def inflictDamage(self):
        ...
        if self.charObj.client:
            ...
            logger.warning(
                "magic.inflictDamage could not deal damage - "
                + "charObj.client == None"
            )
